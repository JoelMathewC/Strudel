...
class RSA{
  ...
  //Encoding Private Key Strings with Public Key
  List<String> dataEncrypt(String data, RSAPublicKey publicKey){
    List<String> encrypted = [];
    int i = 0;

    if(data.length > 255){
      while(i < data.length){
        encrypted.add(encrypt(data.substring(i,i+255 >= data.length ? data.length :i+255),publicKey));
        i = i + 255;
      }
    }
    else{
      encrypted.add(encrypt(data, publicKey));
    }
    return encrypted;
  }
  ...
  String dataDecrypt(List<String> encryptedData, RSAPrivateKey privateKey){
    ...
    for(i = 0;i<encryptedData.length;++i){
      decrypted += decrypt(encryptedData[i],privateKey);
    }
    return decrypted;
  }
